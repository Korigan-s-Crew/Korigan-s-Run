...
int add_character_animation(Character *character, SDL_Texture *texture[], SDL_Rect dst, int nb_frame, int duration) {
...
//            SDL_Rect src = {current_frame * character_animations[i].dst->w, 0, character_animations[i].dst->w,
//                            character_animations[i].dst->h};

            SDL_RenderCopy(renderer, character_animations[i].texture[current_frame], NULL, &character_animations[i].dst);
        }
    }
}
